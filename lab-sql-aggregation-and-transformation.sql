...
SELECT CONCAT(FLOOR(AVG(length) / 60), ':', (ROUND(AVG(length) % 60))) AS avg_duration_formatted FROM film;
...
-- DAYOFWEEK: 1 and 7 are weekend days, 1 = Sunday, 7 = Saturday
SELECT *, 
	CASE
...
SELECT release_year, COUNT(film_id) AS nr_films FROM film GROUP BY release_year ORDER BY release_year;

-- 1.3 The **number of films for each rating, sorting** the results in descending order of the number of films.
-- This will help you to better understand the popularity of different film ratings and adjust purchasing decisions accordingly.
SELECT rating, SUM(film_id) AS num_films FROM film GROUP BY rating ORDER BY SUM(film_id) DESC;
